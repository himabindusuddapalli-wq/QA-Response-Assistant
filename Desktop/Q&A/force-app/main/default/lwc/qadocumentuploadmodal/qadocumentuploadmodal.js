...
export default class QaDocUpload extends LightningElement {
  ...
  connectedCallback() {
    if (!this.jsZipLoaded) {
      loadScript(this, JSZipLib)
        ...
        .catch(err => console.error('JSZip load error:', err));
    }
  ...
  get uploadDisabled() {
    if (this.isUploading || !(this.companyId || this.maId || this.rfpId)) return true;
    ...
    if (this.showExcelMapping) {
      return !this.sheetRenderData.some(
        s => s.isSelected && s.mapping.questionText !== '' && s.mapping.section !== ''
      );
    }
    return false;
  }
  ...
  get processingPdfMessage() {
    return 'Processing PDF with Document AI — this can take up to a minute or more for larger documents. Please don’t close this window.';
  }
 ...
 handleMaPick(event) {
    ...
    getOpportunityDetails({ maId: this.maId })
        ...
        .catch(err => console.error('Opportunity details error:', err));
}
  ...
  handleRfpPick(event) {
    ...
    getRfpDetails({ rfpId: this.rfpId })
      ...
      .catch(err => console.error('RFP details error:', err));
  }
  ...
  async handleUpload() {
    ...
    try {
      ...
      if (ext === 'pdf') {
        ...
        try {
          await uploadAndParsePdfDocument({
          ...
          this.errorMessage = err.body ? err.body.message : 'Upload failed. Please try again.';
          return;
        }
        this.isProcessingPdf = false;
        this.toastMessage = 'PDF submitted. Extraction and answer matching are running in the background — you\'ll receive an email when complete.';
        this.showToast = true;
      ...
      try {
        ...
        console.warn('File parsing skipped:', parseErr);
      }
      ...
      console.log(`Upload: ${totalCount} questions, ${Math.ceil(totalCount / CHUNK) || 1} chunk(s)`);
      ...
      attachFileToDocument({
        ...
        base64Data: base64,
        fileName:   this.fileName
      }).catch(err => console.warn('File attachment failed:', err));

      // Step 4 — kick off answer matching
      const count = totalCount;
      if (count > 0) {
        startAnswerSearchBatch({ documentId: docId }).catch(err => {
          console.error('startAnswerSearchBatch failed:', err);
        });
      }
      ...
      if (count > 0 && (Date.now() - uploadStartTime) > 2000) {
        notifyQuestionsProcessed({ documentId: docId }).catch(err => {
          console.warn('notifyQuestionsProcessed failed:', err);
        });
      }

      this.toastMessage = count > 0
  ...
  _parseExcelFromSheets() {
    ...
    for (const sheet of this.sheetRenderData.filter(s => s.isSelected)) {
      ...
      // When no section column is mapped, sections are inferred from rows where the
      // Q# column has non-numeric text and the question text column is blank
      // (e.g. "Medical Claims Administration" spanning Col A with empty Col B).
      let autoSection = sheet.name; // default section = sheet tab name
      for (let r = startRow; r < sheet.rows.length; r++) {
        ...
        // A row with a non-numeric Q# col value and no question text is a section header.
        ...
        if (secCol < 0) {
          ...
          const section = this._stripBullet((row[secCol] || '').toString().trim())
                              .replace(/\s*-+\s*$/, '');
          if (!section) {
            // No section value on this row — collect consecutive bullet sub-items
            // from the following rows that do have a section.
            let inheritedSec = '';
            const subItems = [];
            let j = r + 1;
            while (j < sheet.rows.length) {
              const nr    = sheet.rows[j] || [];
              const nqRaw = (nr[qCol]   || '').toString().trim();
              const nSec  = this._stripBullet((nr[secCol] || '').toString().trim())
                                .replace(/\s*-+\s*$/, '');
